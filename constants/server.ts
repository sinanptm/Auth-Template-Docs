...
export const serverFolderData: Record<string, FolderInfo> = {
  src: {
    name: "src",
...
import { CLIENT_URL, PORT } from "./config"; // Config variables
import { initConfig } from "./config/initConfig"; // Config initializer
import router from "./presentation/routes"; // API router
...
// Enable CORS with client URL and credentials
...
// Start server and initialize configs
app.listen(PORT, async () => {
  await initConfig(); // Load app configurations
  logger.info(\`Server running at http://localhost:\${PORT} ✅\`, { meta: { skipFile: true } }); // Log startup
  });
          `,
...
import ITokenService from "@/domain/interfaces/services/ITokenService"; // Token service interface
import { inject } from "inversify"; // Dependency injection
import { Services } from "@/di/services"; // Service identifiers
...
export default class AdminAuthMiddleware {
  constructor(@inject(Services.TokenService) private readonly tokenService: ITokenService) {
    this.exec = this.exec.bind(this); // Bind exec method
  }
  ...
  exec(req: CustomRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!authHeader || !authHeader.startsWith("Bearer ")) {
        return res.status(StatusCode.Forbidden).json({ message: "Missing or invalid Bearer token" });
      }
      ...
      // Ensure valid user data and admin role
      if (!id || !email || !role) {
        return res.status(StatusCode.Unauthorized).json({ message: "Incomplete token data" });
      }
      if (role !== UserRole.Admin) {
        return res.status(StatusCode.Forbidden).json({ message: "Admin role required" });
      }
